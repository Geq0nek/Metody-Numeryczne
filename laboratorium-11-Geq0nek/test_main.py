...
def test_custom_integration(a, b, order, result):
    if result is None:
        ...
        result = main.custom_integration(f, a, b, order)
        assert result == pytest.approx(result), f'Spodziewany wynik: {result}, aktualny: {result}. Błędy implementacji.'
...
def test_trapezoidal_rule(a, b, n, result):
    if result is None:
        ...
        result = main.trapezoidal_rule(f, a, b, n)
        assert result == pytest.approx(result), f'Spodziewany wynik: {result}, aktualny: {result}. Błędy implementacji.'
...
def test_rectangular_rule(a, b, n, result):
    if result is None:
        ...
        result = main.rectangular_rule(f, a, b, n)
        assert result == pytest.approx(result), f'Spodziewany wynik: {result}, aktualny: {result}. Błędy implementacji.'